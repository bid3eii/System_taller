...
$page_title = 'Editar Anexo Yazaki #' . $id;

// Fetch Anexo to edit
$stmt = $pdo->prepare("
    SELECT a.*, ps.title as survey_title 
...
// Fetch existing tools for this anexo
...
$existing_tools = $stmtTools->fetchAll(PDO::FETCH_ASSOC);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        // 1. Update main anexo
        $stmtU = $pdo->prepare("UPDATE anexos_yazaki SET survey_id = ?, client_name = ?, updated_at = NOW() WHERE id = ?");
        $stmtU->execute([$survey_id, $client_name, $id]);

        // 2. Delete existing tools (Easiest way to handle varying rows)
        $pdo->prepare("DELETE FROM anexo_tools WHERE anexo_id = ?")->execute([$id]);

        // 3. Re-insert rows from POST array
        ...
        if (isset($_POST['qty']) && is_array($_POST['qty'])) {
            ...
            for ($i = 0; $i < count($_POST['qty']); $i++) {
                ...
                if ($qty > 0 && ($tool_id || $custom_desc !== '')) {
                    $stmtRow->execute([$id, $rowIndex, $tool_id, $custom_desc, $qty]);
                    $rowIndex++;
                }
            }
        }
        ...
        $pdo->commit();
        header("Location: view.php?id=$id&success=edit");
        exit;
    } catch (Exception $e) {
        $pdo->rollBack();
        $error = "Error al actualizar el anexo: " . $e->getMessage();
    }
}
...
<div class="animate-enter">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 2rem;">
        <div>
            <h1>Editar Anexo 10 (Yazaki) #
                <?php echo str_pad($id, 5, '0', STR_PAD_LEFT); ?>
            </h1>
            <p class="text-muted">Modificar las herramientas y detalles del anexo seleccionado.</p>
        </div>
    ...
    <form method="POST" class="card" id="anexoForm">
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem; margin-bottom: 2rem;">
            <div class="form-group">
                <label class="form-label font-medium"><i class="ph ph-projector-screen text-muted"></i> Vincular a un
                ...
                <div style="display: flex; gap: 0.5rem;">
                    ...
                    <input type="text" id="survey_display" class="form-control" value="<?php echo $display_value; ?>"
                        readonly style="cursor: pointer; opacity: 0.9;" onclick="openModal('modalProjects')">

                    <button type="button" class="btn btn-secondary" onclick="openModal('modalProjects')"
                        style="padding: 0.5rem;">
                        <i class="ph ph-magnifying-glass"></i>
                    </button>
                    <button type="button" class="btn btn-secondary" onclick="clearProject()" title="Desvincular"
                        style="padding: 0.5rem;">
                        <i class="ph ph-x"></i>
                    </button>
                </div>
        ...
        <div
            style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid var(--border-color); margin-top: 1rem; margin-bottom: 1rem; padding-bottom: 0.5rem;">
            ...
            <div style="display: flex; gap: 0.5rem;">
                <button type="button" class="btn btn-secondary" onclick="addManualRow()" style="font-size: 0.9rem;">
                    <i class="ph ph-pencil-simple"></i> Agregar Herramienta Manual
                </button>
                <button type="button" class="btn btn-primary" onclick="openModal('modalTools')"
                    style="font-size: 0.9rem; background: var(--primary-600);">
        ...
        <div class="table-container">
            <table id="toolsTable" style="margin-bottom: 1rem;">
                ...
                <tbody id="toolsBody">
                    <!-- Rows injected by JS from preloaded PHP array -->
                </tbody>
            </table>
        </div>
...
<div id="modalProjects" class="modal-overlay">
    <div class="modal-content">
        <div class="modal-header">
            ...
            <button type="button" class="modal-close" onclick="closeModal('modalProjects')"><i
                    class="ph ph-x"></i></button>
        </div>
        <div class="modal-body custom-scrollbar">
            <input type="text" id="searchProjects" class="form-control" placeholder="Buscar por cliente o título..."
                onkeyup="filterTable('searchProjects', 'tableProjects')" style="margin-bottom: 1rem; padding: 0.8rem;">
            <table style="width: 100%;" id="tableProjects">
                <thead>
                    <tr>
                        <th style="text-align: left; padding: 0.5rem;">ID / Título</th>
...
<div id="modalTools" class="modal-overlay">
    <div class="modal-content">
        <div class="modal-header">
            ...
            <button type="button" class="modal-close" onclick="closeModal('modalTools')"><i
                    class="ph ph-x"></i></button>
        </div>
        <div class="modal-body custom-scrollbar">
            <input type="text" id="searchToolsModal" class="form-control"
                placeholder="Buscar herramienta por nombre o tipo..."
                onkeyup="filterTable('searchToolsModal', 'tableTools')" style="margin-bottom: 1rem; padding: 0.8rem;">
            <table style="width: 100%;" id="tableTools">
                <thead>
                    <tr>
                        <th style="text-align: left; padding: 0.5rem;">Herramienta</th>
                        <th style="text-align: left; padding: 0.5rem;">Detalle / Tipo</th>
                        <th style="width: 100px;"></th>
                    </tr>
                </thead>
                <tbody>
...
<!-- Outputting current tools as JSON array for JS load -->
<script>
    const INITIAL_TOOLS = <?php echo json_encode($existing_tools); ?>;
</script>
...
<script>
    ...
    // Pre-load Existing Tools
    document.addEventListener('DOMContentLoaded', () => {
        INITIAL_TOOLS.forEach(tool => {
            if (tool.tool_id) {
                addToolRowWithValues(tool.tool_id, tool.tool_name, tool.quantity);
            } else {
                addManualRowWithValues(tool.custom_description, tool.quantity);
            }
        });
    });

    // Modified insertion to handle prepopulated values
    function addToolRowWithValues(id, name, quantity) {
        rowCount++;
        let tr = document.createElement('tr');
        tr.innerHTML = '<td style=\"text-align: center; font-weight: bold;\" class=\"row-number\">' + rowCount + '</td>' +
            '<td><input type=\"number\" step=\"0.01\" min=\"0\" name=\"qty[]\" class=\"form-control qty-input\" value=\"' + quantity + '\" style=\"padding: 0.4rem; font-size: 0.9rem;\"></td>' +
            '<td>' +
    ...
    function addManualRowWithValues(desc, quantity) {
        rowCount++;
        let tr = document.createElement('tr');
        tr.innerHTML = '<td style=\"text-align: center; font-weight: bold;\" class=\"row-number\">' + rowCount + '</td>' +
            '<td><input type=\"number\" step=\"0.01\" min=\"0\" name=\"qty[]\" class=\"form-control qty-input\" value=\"' + quantity + '\" style=\"padding: 0.4rem; font-size: 0.9rem;\"></td>' +
            ...
            '<input type=\"text\" name=\"desc[]\" class=\"form-control\" value=\"' + desc + '\" placeholder=\"Describe el artículo manual...\" style=\"padding: 0.4rem; font-size: 0.9rem;\" required>' +
            '</td>' +
            '<td style=\"text-align: center;\"><button type=\"button\" class=\"btn-icon\" style=\"color: var(--danger); background: transparent; border: none; cursor: pointer;\" onclick=\"removeRow(this)\" title=\"Quitar\"><i class=\"ph ph-trash\"></i></button></td>';
        document.getElementById('toolsBody').appendChild(tr);
    }

    // Original Insertions for runtime usage
    function addToolRow(btn, id, name) {
        addToolRowWithValues(id, name, 1);
        reindexRows();
